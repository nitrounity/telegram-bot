...
function isAdmin(userId) {
  return normalizeUserId(userId) === adminId
}

function isTestUser(userId) {
  return testUsers.has(normalizeUserId(userId))
}

// 🔹 CHECK IF USER PAID
async function hasPaid(userId) {
  if (isAdmin(userId) && isTestUser(userId)) {
    return true
  }
...
bot.start(async (ctx) => {
  ...
  userMap.set(normalizeUserId(userId), username)
  ...
  if (paid) {
    const inGroup = isTestUser(userId) ? false : await isUserInGroup(userId)
...
bot.action(/reply_(.+)/, async (ctx) => {
  ...
  const username = userMap.get(normalizeUserId(userId)) || "user"
...
bot.command('test', (ctx) => {
  ...
  testUsers.add(normalizeUserId(ctx.from.id))
  return ctx.reply("🧪 Test ON. You can now use /access to generate a test invite link.")
})

bot.command('stoptest', (ctx) => {
  if (!isAdmin(ctx.from.id)) return

  testUsers.delete(normalizeUserId(ctx.from.id))
  return ctx.reply("🛑 Test OFF")
})
